...
Checkout_TC_1: To Verfiy guest customer is able to see checkout page
    ...
    #Verify item price --> comment this section if have the discount
    ${SHIPPING_ORDER_INFORMATION_SECTION_ITEM_LIST_ITEM_PRICE}    Get Text    ${SHIPPING_ORDER_INFORMATION_SECTION_PRICE}
    Log To Console    Original price is ${SHIPPING_ORDER_INFORMATION_SECTION_ITEM_LIST_ITEM_PRICE}
    # ----------------------- Uncomment this section if have the discount ---------------------------------
...
Checkout_TC_3: To verify registered customer able to create new shipping address in Shipping Details tab during the checkout process.
    [Documentation]    Add new address (Have to manual add address before run automate)
    Open Website
    Accept Cookies
    Login    ${LOGIN_EMAIL_EXISTING_CUSTOMER}      ${LOGIN_PASSWORD_EXISTING_CUSTOMER}
    #Click on Search icon
